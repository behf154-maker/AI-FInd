  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // Insert user with new fields (including optional phone and class)
        await connection.execute(
          'INSERT INTO users (name, email, password, role, school, grade, phone, class) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [name, email, hashedPassword, role || 'student', school || null, grade || null, phone || null, studentClass || null]
        );

        // Get user ID and role
        const [newUser] = await connection.execute('SELECT user_id, role FROM users WHERE email = ?', [email]);
        connection.release();

        const userRole = newUser[0]?.role || 'student';
