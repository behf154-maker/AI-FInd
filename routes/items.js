...
module.exports = (pool, upload) => {
  ...
  router.post('/request-pickup', verifyToken, async (req, res) => {
    try {
      ...
      // Also create notification for admin with student name, email, and item details
      ...
      for (const admin of admins) {
        await connection.execute(
          ...
          [
            admin.user_id,
            found_item_id,
            `Pickup Request: Student ${requesterName} (${requesterEmail}) wants the item: ${foundItem.item_type} - ${foundItem.color} - ${foundItem.size} from ${foundItem.location_found || 'unknown location'}`
          ]
        );
      }
